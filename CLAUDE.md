...
## 11. The contract with studio-ops-web

`openapi.json` is generated by Scramble from the controllers, Form Requests and
API Resources. It is committed, and `composer openapi:check` — which CI runs —
regenerates it and fails if the result differs.

The contract is generated, never edited. Changing a validation rule changes the
contract, and `studio-ops-web` has tests that check its Zod schema against the
published limits, so a rule moved here fails the build over there. That is the
